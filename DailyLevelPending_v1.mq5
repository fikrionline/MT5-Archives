...
input long TrailingStartPoints = 19000000;  // start trailing after this many points in profit
input long TrailingStepPoints  = 19000000;  // step size for moving stop
...
void OnTick() {
   ...
   double dailyHigh = iHigh(_Symbol, TimeFrame, 1);
   double dailyLow = iLow(_Symbol, TimeFrame, 1);
   ...
   if (!HasPendingAtPrice(ORDER_TYPE_SELL_LIMIT, dailyHigh))
      PlacePending(ORDER_TYPE_SELL_LIMIT, h1High, StopLossPoints, h1High, h1Low);
   if (!HasPendingAtPrice(ORDER_TYPE_BUY_LIMIT, dailyLow))
      PlacePending(ORDER_TYPE_BUY_LIMIT, h1Low, StopLossPoints, h1High, h1Low);
}
...
void PlacePending(ENUM_ORDER_TYPE type, double price, long StopLossPoints, double h1High, double h1Low) {
   double lots = CalculateLot(StopLossPoints);
   if (lots < MinLot) lots = MinLot;
   if (lots > MaxLot) lots = MaxLot;

   datetime expiration = TimeCurrent() + ExpirationHours * 3600;

   double stopDistance = (h1High - h1Low);
   double sl = 0, tp = 0;
   if (type == ORDER_TYPE_SELL_LIMIT) {
      sl = price + stopDistance;
      if (UseTakeProfit)
         tp = price - (stopDistance * RRRatio);
   ...
   if (!OrderSend(request, result)) {
      ...
      PrintFormat("Pending placed ticket=%I64u price=%.5f lots=%.2f SL=%.5f TP=%.5f", result.order, request.price, lots, request.sl, request.tp);
}
